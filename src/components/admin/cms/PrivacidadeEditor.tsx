...
import { Save, Loader2, ArrowLeft, AlertCircle, LayoutTemplate, Plus, Trash2, ChevronDown, ChevronUp } from 'lucide-react';
...
interface PrivacidadeEditorProps {
    siteId: string;
    repoName: string;
    siteUrl: string;
}

export default function PrivacidadeEditor({ siteId, repoName, siteUrl }: PrivacidadeEditorProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (!res.ok) throw new Error('Erro ao buscar privacy.json.');
    ...
    const handleSave = async (e?: React.FormEvent) => {
        ...
        triggerToast('Salvando Política de Privacidade...', 'progress', 20);
        ...
        try {
            ...
            const res = await fetch('/api/cms/github', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    action: 'write', repo: repoName, path: 'src/data/privacy.json',
                    content: JSON.stringify(updatedData, null, 2), sha: fileSha,
                    message: 'CMS: Atualização da Política de Privacidade'
                })
            });
            ...
            triggerToast('Privacidade atualizada! Processando build...', 'progress', 60);
            let prog = 60;
            ...
            setTimeout(() => {
                ...
                triggerToast('Página publicada com sucesso!', 'success', 100, formattedUrl + '/privacidade');
                setSaving(false);
            }, 12000);
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-32 text-slate-400 bg-white">
            ...
            <p className="font-semibold text-sm animate-pulse text-slate-500">Buscando privacy.json...</p>
        </div>
    );
                ...
                <div className="bg-white p-5 rounded-sm border border-slate-200">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="text" value={data?.seo?.title || ''} onChange={e => updateSeo('title', e.target.value)} className={inputClass} placeholder="Privacidade | SEO" />
                        </div>
                        <div>
